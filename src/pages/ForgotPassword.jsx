...
export const ForgotPassword = () => {
  ...
  const handleSubmit = (event) => {
    try {
      ...
      const response = dispatch(sendResetPasswordToken({ email }));
      if (response !== null) {
        setisUrlSend(true);
      } else {
        setisUrlSend(false);
    ...
    <div className="bg-richblack-900 flex items-center justify-center h-screen">
      {isUrlSend === true ? (
        <div className="flex items-start gap-3 flex-col text-richblack-50 w-[23rem]">
            ...
            <div className="text-richblack-200">We have sent the reset email to
            youremailaccount@gmail.com</div>
            </div>
            <button onClick={handleSubmit} className="flex w-[23rem] py-2 px-2 bg-yellow-100 items-center justify-center text-richblack-800 rounded-xl">Resend Email</button>
        ...
        <div className="flex items-start gap-3 flex-col text-richblack-50 w-[23rem]">
          ...
          <form onSubmit={handleSubmit} className="flex flex-col items-start w-full gap-3">
            ...
            />
            <button type="submit" className="flex w-[23rem] py-2 px-2 bg-yellow-100 items-center justify-center text-richblack-800 rounded-xl">Reset Password</button>
          </form>
          <Link to="/login">Back to login</Link>
        </div>
      )}
